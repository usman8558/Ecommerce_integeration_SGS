...
from ecommerce_integrations.shopify.connection import get_dynamic_access_token
...
class ShopifySetting(SettingController):
	...
	def _handle_webhooks(self):
		if self.is_enabled() and not self.webhooks:
			dynamic_token = get_dynamic_access_token(self)
			new_webhooks = connection.register_webhooks(self.shopify_url, dynamic_token)
			...
			dynamic_token = get_dynamic_access_token(self)
			connection.unregister_webhooks(self.shopify_url, dynamic_token)
